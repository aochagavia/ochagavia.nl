...
date = "2025-04-28"
tags = ["HN frontpagers"]
hn = "https://news.ycombinator.com/item?id=43826586"
+++
...
- Otherwise: the receiver will _not_ detect that the `conversation_id` has been tampered with. Instead, it will attempt to decrypt the message and handle it as usual. If the modified `conversation_id` corresponds to a conversation between the same two users as before, then decryption will succeed and the message will arrive to a different conversation than the one the sender intended! This, then, illustrates how lack of authentication of associated data can expose you to an attack.
...
send(nonce, ciphertext, associated_data)
