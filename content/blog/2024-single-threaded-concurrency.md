...
The secret sauce is in `wait_until_next_poll().await`. Every time a partial execution reaches this line, it will stop and wait until the next call to `poll`. Remove that line, and the first call to `poll` will cause the whole function to be executed in one go.
